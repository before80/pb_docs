...
The Java package name for each file is the same as that used by the generated Java code as described in the [Java generated code reference](https://protobuf.dev/reference/java/java-generated#package).
...
Note that properties always use camel-case naming, even if the field name in the `.proto` file uses lower-case with underscores ([as it should](https://protobuf.dev/programming-guides/style)). The case-conversion works as follows:
...
- `val oneofNameCase: OneofNameCase`: gets which, if any, of the `oneof_name` fields are set; see the [Java code reference](https://protobuf.dev/reference/java/java-generated#oneof) for the return type
